...
def make_router(
    *,
    ...
    async def auto_set_receivers(request: Request):
        ...
        snr_thresholds = _fetch_snr_by_band(host, port)
        ...
        rx_display_list = [2, 3, 4, 5, 6, 7, 0, 1]
        rx_request_list = [0, 1, 2, 3, 4, 5, 6, 7]
        desired_tasks = tasks[: len(rx_display_list)]

        assignments: Dict[int, ReceiverAssignment] = {}
        assignment_results: List[Dict[str, object]] = []
        allowed_bands: set[str] = set()

        for i, t in enumerate(desired_tasks):
            rx_request = int(rx_request_list[i])
            rx_display = int(rx_display_list[i])
