...
_AUTOMATION_SETTINGS_PATH = Path(__file__).resolve().parents[3] / "outputs" / "automation_settings.json"


def _load_automation_settings() -> Dict[str, object]:
    try:
        if not _AUTOMATION_SETTINGS_PATH.exists():
            return {}
        data = json.loads(_AUTOMATION_SETTINGS_PATH.read_text(encoding="utf-8"))
...
def _save_automation_settings(payload: Dict[str, object]) -> None:
    try:
        _AUTOMATION_SETTINGS_PATH.parent.mkdir(parents=True, exist_ok=True)
        _AUTOMATION_SETTINGS_PATH.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    except Exception:
        pass


def _external_hold_reason(auto_set_loop: object | None) -> str | None:
...
def make_router(
    *,
    ...
    async def auto_set_receivers(request: Request):
        ...
        external_hold_reason = _external_hold_reason(auto_set_loop) if enabled else None
