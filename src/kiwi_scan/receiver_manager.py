...
class ReceiverManager:
    ...
    def _roaming_correction_plan(
        self,
        *,
        live_users: Dict[int, str],
        assignments: Dict[int, ReceiverAssignment],
        roaming_rxs: list[int],
    ) -> tuple[list[int], list[int], Dict[int, int]]:
        expected_roam = {
            int(rx): self._expected_user_label_aliases(assignments[int(rx)])
            for rx in roaming_rxs
            if int(rx) in assignments
        }
        actual_slots: Dict[int, int] = {}
        for slot, label in live_users.items():
            for rx, expected_labels in expected_roam.items():
                if self._label_matches_any(expected_labels, label):
                    actual_slots.setdefault(int(rx), int(slot))
                    break

        relaxed_low_slot_pool = bool(roaming_rxs) and all(int(rx) < 2 for rx in roaming_rxs)
        if relaxed_low_slot_pool:
            expected_any = list(expected_roam.values())
            restart_rxs = [int(rx) for rx in roaming_rxs if int(rx) not in actual_slots]
            kick_slots = [
                int(slot)
                for slot in roaming_rxs
                if int(slot) in live_users
                and not any(self._label_matches_any(labels, live_users[int(slot)]) for labels in expected_any)
            ]
            return sorted(set(restart_rxs) | set(kick_slots)), sorted(set(kick_slots)), actual_slots

        needs_fix = [
            int(rx)
            for rx in roaming_rxs
            if actual_slots.get(int(rx)) != int(rx)
        ]
        blocked_targets = [
            int(slot)
            for slot in roaming_rxs
            if int(slot) in live_users
            and not self._label_matches_any(expected_roam[int(slot)], live_users[int(slot)])
        ]
        restart_rxs = sorted(set(needs_fix) | set(blocked_targets))
        kick_slots = sorted(
            set(restart_rxs)
            | {
                int(slot)
                for rx, slot in actual_slots.items()
                if int(rx) in restart_rxs
            }
        )
        return restart_rxs, kick_slots, actual_slots
        ...
        with self._lock:
            ...
            def _start_fixed_bootstrap_placeholder(rx: int, template_assignment: ReceiverAssignment) -> None:
                placeholder = ReceiverAssignment(
                    ...
                    user_label_override=f"HOLD_RX{int(rx)}",
                )
            ...
            if roaming_rxs_to_verify and (not starting_from_empty or _defer_roaming_start):
                ...
                for _roam_attempt in range(MAX_ROAMING_CORRECTION_RETRIES):
                    ...
                    _rxs_to_restart, _slots_to_kick, _actual_slots = self._roaming_correction_plan(
                        live_users=_live_roam,
                        assignments=assignments,
                        roaming_rxs=roaming_rxs_to_verify,
                    )
