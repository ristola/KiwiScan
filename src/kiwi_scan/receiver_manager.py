            ...
            try:
                ...
                if isinstance(payload, list):
                    for row in payload:
                        ...
                        if not label_match:
                            # KiwiSDR firmware may relabel connections at FT8 dial
                            # frequencies to AUTO_<band>_FT8 regardless of the user
                            # name sent by the client.  Fall back to slot+frequency
                            # matching so these workers are not endlessly restarted.
                            try:
                                slot = int(row.get("i", -1))
                                freq = float(row.get("f", 0))
                                if not (
                                    name.startswith("AUTO_")
                                    and slot == self._kiwi_rx_chan()
                                    and abs(freq - float(self._freq_hz)) <= 500
                                ):
                                    continue
                            except Exception:
                                continue
        ...
        while time.time() < deadline and not self._stop_event.is_set():
            try:
                ...
                for row in payload:
                    ...
                    label_match = name == wanted or name.startswith(wanted) or truncated_match
                    if not label_match:
                        # KiwiSDR firmware may relabel connections at FT8 dial
                        # frequencies to AUTO_<band>_FT8.  Accept the firmware-
                        # assigned label when the expected slot and frequency match.
                        try:
                            slot = int(row.get("i", -1))
                            freq = float(row.get("f", 0))
                            if not (
